...
_modules = ["api"]
_seed_module = _add_module_prefix("api")

tmodules = list(map(_add_module_prefix, _modules))
modules = _modules + tmodules
groups = {"all": tmodules, "main": list(map(_add_module_prefix, ["api"])), "seed": list(map(_add_module_prefix, ["api"])) }
hosts = [
    "backend00.company.int"
]

environments = {
     "dev": [
        {
            "host": "backend00.company.int",
            "modules": list(map(_add_module_prefix, ["api"]))
        }
    ]
}
...
actionParser.add_argument("-a", "--action", dest="action", help = "action for module", 
    choices=["stop", "start", "restart", "status"])

modulesParser = argparse.ArgumentParser(add_help = False)
modulesParser.add_argument("-m", "--modules", nargs="+", default = [],  help = "backend modules to be installed", type=_module_check).completer=ModuleCompleter

groupsParser = argparse.ArgumentParser(add_help = False)
groupsParser.add_argument("-g", "--groups", nargs="+", default = [], help = "group of backend modules to be installed", type=_group_check).completer=GroupCompleter

hostParser = argparse.ArgumentParser(add_help = False)
hostGroup = hostParser.add_mutually_exclusive_group()
hostGroup.add_argument("-t", "--target", dest="target", help = "target host", choices=hosts)
hostGroup.add_argument("-e", "--env", dest="env", default="dev", help = "target environment", choices=list(environments.keys()))
